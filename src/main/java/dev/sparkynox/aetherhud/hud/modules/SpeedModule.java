...
public class SpeedModule extends HudModule {
    ...
    public SpeedModule(float x, float y) { super("speed", x, y); }
    ...
    public void render(DrawContext ctx, float tickDelta) {
        ...
        smooth += ((float)(Math.sqrt(dx*dx+dz*dz)*20f) - smooth) * 0.15f;

        AetherDraw.drawCard(ctx, 0, 0, getWidth(), getHeight());
        AetherDraw.drawAccent(ctx, 0, 0, getHeight());
        AetherDraw.drawIconSpeed(ctx, 5, 5, AetherDraw.PURPLE);
        ctx.drawText(font, "SPD",                       16, 2,  AetherDraw.LABEL, false);
        ctx.drawText(font, String.format("%.1f",smooth), 16, 11, AetherDraw.VALUE, false);
    }
